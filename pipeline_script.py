...
class ConcertModel:
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, model_type: str = 'rf',
        ...
        if use_cv and len(self.X_train) >= cv_folds:
            ...
            logger.info("Skipping cross-validation due to small training set")
...
class PipelineOrchestrator:
    ...
    def run(self, output_dir: str = '.') -> Dict:
        ...
        if self.use_external_data:
            ...
            fetch_all_external_data(self.raw_csv, output_dir)
            try:
        ...
        X = df.drop(columns=['event_id', 'attendance', 'attendance_rate'], errors='ignore')
